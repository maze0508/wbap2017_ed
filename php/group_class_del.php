...
if($button_type=="image"){
	$query="UPDATE media_image SET anchor_class_id='' WHERE anchor_class_id='$group_class_id'";
	$result = $mysqli->query($query);

	$query="DELETE from group_class WHERE group_class_id='$group_class_id'";
	...
	echo"<div id='' class='class_anchor'>";
		...
		//顯示小組圖片註記分類
		$query="SELECT member.name, media_image.media_image_id, media_image.image, media_image.noteColor, media_image.anchor_time, group_class.group_class_id, group_class.anchor_class_name
			FROM member
			LEFT JOIN media_image ON member.member_id = media_image.member_id
			LEFT JOIN group_class ON media_image.anchor_class_id = group_class.group_class_id
			WHERE media_image.user_media_id =  '$user_media_id'
		...
		while($row = $result->fetch_array(MYSQL_ASSOC)){
			...
			if($noteColor==0){
				...
				echo "<div id='$media_anchor_id' class='Note' name='未分類'>
					...
					<table><tr><td style='width:165px;'><div name='$group_class_id' class='select_class' >$class_name</div></td></tr></table></div>
				...
				echo "<div id='$media_anchor_id' class='Note_other' name='未分類'>
					<div name='$class_name' class='select_edit'>
						<table>
							<tr>
								<td style='width:110px;'><div name='$group_class_id' class='select_class' >$class_name</div></td>
	...
	$query="UPDATE group_anchor SET group_class_id='' WHERE group_class_id='$group_class_id'";
	$result = $mysqli->query($query);
