			...
			if(empty($row)){
				...
				while($row) { 
					...
					//Youtube影片的註記內容，因為無法截圖，故不顯示圖片	
						...
						echo "<li id='$media_anchor_image_id'>
							<div style='width:100%;color:#69C;'><a style='text-decoration: none;' href='../m/start_learning_1.php?user_media_id=$user_media_id&team_id=$team_id&anchor_time=$anchor_time'><div id='$anchor_time' class='antime $anchor_time' style='font-size:12pt;'>註記時間：[$h:$m:$s]</div><div><img class='image' style='width:75%;height:75%;' src='../images/anchor/$image'/></div>
							<div style='font-size:10pt;'>註記內容：$anchor_descript</div></a>
							<div><img class='delete_button' style='width:30px;'src='../images/cancel.png';></img></div></div>
						</li>";
					}}
					$row = $result->fetch_array(MYSQL_ASSOC);
				}
